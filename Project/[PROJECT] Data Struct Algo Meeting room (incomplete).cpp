...
// LINKED LIST FOR MEETING ROOM DATABASE
...
public:
    ...
    void addRoom(const string& name, bool available, int capacity) {
        Room* newRoom = new Room{name, available, capacity};
        if (head == nullptr) {
            head = newRoom;
        }
        else {
...
int main() {
    ...
    if (id == "1211206128" || password == "123456") {
		...
		roomList.addRoom("Room 1", true, 10);
        roomList.addRoom("Room 2", true, 8);
        roomList.addRoom("Room 3", false, 12);
